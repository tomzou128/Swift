...
```self``` is created automatically in Struct and defined with the "let" keyword. If we have a function which changes the variable of the struct. Then we need to have a ```mutating``` keyword.
```swift
