...
We can then use it in a structure. Note that system will automatically provide initializer for a structure. But since we don't want to modify the ```unit_price``` from outside, we must write our own initialier.
